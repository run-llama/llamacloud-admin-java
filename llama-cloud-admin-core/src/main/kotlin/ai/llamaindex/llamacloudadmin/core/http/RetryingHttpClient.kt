...
import ai.llamaindex.llamacloudadmin.errors.LlamaCloudAdminIoException
import ai.llamaindex.llamacloudadmin.errors.LlamaCloudAdminRetryableException
...
private constructor(
    ...
    private fun shouldRetry(throwable: Throwable): Boolean =
        ...
        throwable is IOException || throwable is LlamaCloudAdminIoException || throwable is LlamaCloudAdminRetryableException

    private fun getRetryBackoffDuration(retries: Int, response: HttpResponse?): Duration {
        // About the Retry-After header:
        // https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Retry-After
        response?.headers()?.let { headers ->
            headers.values("Retry-After-Ms").getOrNull(0)?.toFloatOrNull()?.times(TimeUnit.MILLISECONDS.toNanos(1))
