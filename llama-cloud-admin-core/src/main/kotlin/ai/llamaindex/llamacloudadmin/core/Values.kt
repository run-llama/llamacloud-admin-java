...
sealed class JsonField<out T : Any> {
    ...
    internal fun getRequired(name: String): T =
        when (this) {
            is KnownValue -> value
            is JsonMissing -> throw LlamaCloudAdminInvalidDataException("`$name` is not set")
            is JsonNull -> throw LlamaCloudAdminInvalidDataException("`$name` is null")
            else -> throw LlamaCloudAdminInvalidDataException("`$name` is invalid, received $this")
        }
